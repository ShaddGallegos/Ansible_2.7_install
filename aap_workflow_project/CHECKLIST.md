...
- [x] Automation gateway readiness path updated with loopback-aware TLS handling and diagnostics.
- [x] Automation gateway service-node gather path switched away from restricted helper container execution.
- [x] Automation gateway route update loop restored to prevent `item is undefined` route failures.
- [x] Receptor signing key distribution switched from fetch to slurp plus content-copy to avoid `/tmp/ansible.*` permission errors.
...
- [ ] Re-run workflow install with latest collection patch overlay and capture next failing task if any remain.
