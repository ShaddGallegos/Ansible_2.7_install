...
echo "[2/4] Patching state.sh to support lowercase state keys in env.yml..."
# Fix case-sensitivity regex so rhsm_user_B64 and rhsm_password_B64 are correctly decoded
sed -i 's/\[A-Z\]\[A-Z0-9_\]\*/[A-Za-z0-9_]*/g' "$STATE_LIB"
sed -i 's/\$helper" set-value/\$helper" set/g' "$STATE_LIB" 2>/dev/null || true

echo "[3/4] Patching aap27_installer.sh to force credential prompt when missing..."
python3 - << 'PYEOF'
import re
...
# Make missing .env file set -e safe
content = re.sub(
...
# Force non-interactive flag to false during credential prompt if credentials are missing
...
echo "[4/4] Adding interactive fallback prompt handler to lib/state.sh..."
